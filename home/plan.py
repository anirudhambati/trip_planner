...
def bipartite_Graph(M, bipartite_set, odd_vertices):
	...
	for vertex_set1 in bipartite_set:
		...
		#print(len(vertex_set2), len(vertex_set1))
		...
		for i in range(len(vertex_set1)):
			for j in range(len(vertex_set2)):
				if vertex_set1[i] < vertex_set2[j]:
					#print(vertex_set1[i], vertex_set2[j])
...
def create_Multigraph(M, MST, indexes, odd_vertices):
	...
	multigraph.add_weighted_edges_from(MST)
	for pair in indexes:
		multigraph.add_edge(pair[0],pair[1],weight=M[pair[0]][pair[1]])
	#print(multigraph.edges)
	return multigraph.edges

def Euler_Tour(multi):
	""" Uses Fleury's algorithm to find the Euler Tour of the MultiGraph.

	"""
	multigraph = nx.MultiGraph()
	multigraph.add_weighted_edges_from(multi)
	tour = []
	temp_graph = nx.MultiGraph()
	graph_nodes = multigraph.nodes
	current_node = graph_nodes[0]
	tour.append(current_node)
	while len(multigraph.edges) > 0:
		for edge in multigraph.edges:
			temp_graph_edges = copy.deepcopy(multigraph.edges)
			temp_graph.add_weighted_edges_from(temp_graph_edges)
...
if __name__ == "__main__":
	start = time.time()
	nodes = [('Hyderabad', 'Banglore', 1800), ('Hyderabad', 'Chennai', 500), ('Hyderabad','Mumbai', 1500), ('Banglore','Mumbai', 1700), ('Banglore','Chennai', 1900), ('Chennai', 'Mumbai', 700)]
	graph = nx.Graph()
	graph.add_weighted_edges_from(nodes)
	plt.subplot(111)
	nx.draw(graph, with_labels=True, font_weight='bold')
	plt.show()
	distance_mat = nx.adjacency_matrix(graph)
	distance_mat = distance_mat.todense().tolist()
	nodes = list(graph.nodes)
	Approximation =  compute(distance_mat)
	end = time.time()-start
	path = [nodes[i] for i in Approximation['Christofides_Solution']]
